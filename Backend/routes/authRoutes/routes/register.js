...
function verifyEmail(req, res, next) {
	...
	else res.json({ message: "Invalid Email Id" });
}

function verifyPhoneNumber(req, res, next) {
	const { phone_number } = req.body;
	if (/\d{10}/g.test(phone_number)) next();
	else res.json({ message: "Invalid Phonenumber" });
}
...
function verifyPassword(req, res, next) {
	...
	if (
		...
		next();
	else res.json({ message: "Not a Strong Password" });
}

router.post(
	"/",
	[verifyEmail, verifyPhoneNumber, verifyPassword],
	(req, res) => {
		try {
			const { name, password, phone_number, email, type } = req.body;
